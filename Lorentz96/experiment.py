...
def dXdt(X, N):
    ...
    Xm1 = torch.roll(X, shifts=1,  dims=0)
    Xp1 = torch.roll(X, shifts=-1, dims=0)
    Xm2 = torch.roll(X, shifts=2,  dims=0)

    R = Xm1 * Xp1 - Xm2 * Xm1 - X + F
    return R
...
def RK_Lorentz96_f(X, N_exp, N, delta_t):
    """
    Parameters
    ----------
    X : (N_exp, d, N)
...
def forecast(X, N, N_ob, delta_t, sigma_f):
    ...
    X_return = X.clone()
    for _ in range(N_ob):
        X_return = RK_Lorentz96(X_return, N, delta_t)
...
def forecast_f(X, N_exp, N, N_ob, delta_t, sigma_f):
    ...
    X_return = X.clone()
    for _ in range(N_ob):
        X_return = RK_Lorentz96_f(X_return, N_exp, N, delta_t)
    return X_return + sigma_f * torch.randn_like(X_return, dtype=torch.float64)
...
def enkf_inflation(N_exp, X_sobol, N, delta_t, N_ob, N_assimilation,
                   ob, sigma_ob, sigma_f, inflation, reg=1e-3, device=None):
    """
    Parameters
    ----------
    N_exp : int
    ...
    for n_assi in range(N_assimilation):
        ...
        rho = torch.sqrt(torch.tensor(inflation, device=device, dtype=torch.float64))
        X_b = rho * X_b
        X_f = mu_f + X_b
...
def plain_kmed_inflation(N_exp, X_sobol, N, delta_t, N_ob, N_assimilation,
               ob, sigma_ob, sigma_f, Ns_kmed, band, inflation, reg=1e-3, device=None):
    """
    Parameters
    ----------
    N_exp : int
...
def kakmed_inflation(N_exp, X_sobol, N, delta_t, N_ob, N_assimilation,
           ob, sigma_ob, sigma_f, Ns_kmed, band, inflation, reg=1e-3, device=None):
    """
    Parameters
    ----------
