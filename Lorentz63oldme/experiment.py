...
def enkf_inflation(N_exp, mu_0, C_0, N, delta_t, N_ob, N_assimilation,
                   ob, sigma_ob, sigma_f, inflation, device=None):
    """
    Parameters
    ----------
    N_exp : int
    ...
    Returns
    -------
    X_prior_path : (N_exp, N_assimilation, d, N)
    X_path : (N_exp, N_assimilation + 1, d, N)
    """
    if device is None:
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    mu_0 = mu_0.to(device, dtype=torch.float64)
    C_0 = C_0.to(device, dtype=torch.float64)
    ob = ob.to(device, dtype=torch.float64)

    R = (sigma_ob ** 2) * torch.eye(d, device=device, dtype=torch.float64)

    X_sobol = sobol_multivariate_normal(N, mu_0, C_0).to(device)

    X_path = torch.zeros((N_exp, N_assimilation + 1, d, N), device=device, dtype=torch.float64)
    X_prior_path = torch.zeros((N_exp, N_assimilation, d, N), device=device, dtype=torch.float64)
    X_path[:, 0, :, :] = X_sobol.unsqueeze(0).expand(N_exp, -1, -1)
...
def plain_kmed(N_exp, X_sobol, N, delta_t, N_ob, N_assimilation,
    ...
    for n_assi in range(N_assimilation):
        ...
        for _ in range(Ns_kmed):
            ...
            k, grad1k = kernel_f(X_a, sigma = band)
