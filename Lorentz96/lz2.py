...
if __name__ == '__main__':
    ...
    for j in range(inflation.shape[0]):
        X_po = plain_kmed_inflation(N_exp, pr, N, delta_t, N_ob, N_assimilation, ob_n, sigma_ob, sigma_f, Ns_kmed, alpha, inflation[j], reg = 1e-3)
        po_pk.append(np.array(X_po.cpu()))

    np.savez("Data/Lorentz96/pk_lr.npz", *po_pk)
